...
$(document).ready(async function () {
	...
	function loadHistory(){
		console.log(userHis);
		historyRec = userHis;
		loadCount = historyRec.length -1;
		firstC = loadCount - 10;
		
		for (i = loadCount; i > firstC; i--){
			if(typeof historyRec[i] === 'undefined'){
				console.log("end of history");
			}
			else{
				//console.log(historyRec[i]);
				firebase.firestore()
				.collection("NewsArticleData").doc(historyRec[i]).get().then(function(doc) {
				if (doc.exists) {
					printNews(doc.data(), doc.id);
	...
	function printNews(value, ID) {
		...
		document.getElementById ("unsave" + ID).addEventListener ("click", function () {removeHis(ID)}, false);
